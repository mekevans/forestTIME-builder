...
data_midpt <-
  ...
  #just pick one year for example
  filter(YEAR == 2005)
```
...
Now, these numbers are quite a bit larger than using `TPA_UNADJ` (as shown below).  I found this tidbit in the "Green Book" that may explain it:

>ao = total area normally used to observe the attribute of interest on a plot (i.e., ***four times the microplot, subplot, or macroplot area***)
> --pg 53, describing eq 4.8, emphasis mine

If I multiply the plot area by 4, I get a number much closer to the one in the next section.  But this begs the question: **why is four times the plto area normally used?**

```{r}
data_plot_total |> 
    #calculate carbon/area for each plot
    mutate(carbon_ton_acre = as.numeric(carbon_total / (area_acre * 4))) |> #use area * 4 as denominator
    #get summary stats
    summarize(
        mean_carbon_ton_acre = mean(carbon_ton_acre),
        sd_carbon_ton_acre = sd(carbon_ton_acre),
...
data_carbon_lb_acre |>
  group_by(plot_ID) |>
  summarize(
    carbon_lb_acre = sum(carbon_lb_acre, na.rm = TRUE),
    .groups = "drop"
  ) |>
  #unit conversion
...
So, these are obviously **COMPLETELY** different numbers, by several orders of magnitude. I think it would be important to know why!
