...
Yeah, that seems to have done it! Then we can just cut off any dead trees at the estimated mortality year.
...
I *think* only `ACTUALHT` and `DIA` need interpolation?  We should be able to get rid of most everything in the `tree` table when creating annualized measures.  If users need the per-tree data, they can join back with the `tree` table by TREE_COMPOSITE_ID.
...
There are some trees where interpolation fails.  For example:
