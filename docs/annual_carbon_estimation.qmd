...
nsvb_vars <- nsvb_vars |> 
  left_join(
    ref_species |>
      ...
      rename(WDSG = WOOD_SPGR_GREENVOL_DRYWT), #I assume this is the variable some carbon estimation function is looking for
    by = join_by(SPCD)
  )
```
...
There are no notes in the code indicating why this is done using `DECAYCD` 3:
