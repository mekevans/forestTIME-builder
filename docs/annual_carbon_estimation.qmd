...
all_yrs <- tree_to_interpolate |> 
  ...
  # expand(YEAR = seq.int(min(INVYR, na.rm = TRUE), max(INVYR, na.rm = TRUE)))

#may have better luck with computational issues by chunking this by groups of plots
plot_chunks <- tree_to_interpolate |> ungroup() |> select(PLOT_COMPOSITE_ID) |> distinct() |> mutate(plot_chunk = ntile(PLOT_COMPOSITE_ID, n = 10))

chunk_list <- 
  left_join(tree_to_interpolate, plot_chunks) |>
  group_by(plot_chunk) |> 
  group_split()
...
tree_annual <- 
  right_join(
    tree_to_interpolate,
    all_yrs,
    by = c("TREE_COMPOSITE_ID", "PLOT_COMPOSITE_ID", "SPCD", "INVYR" = "YEAR")
  ) |>
  arrange(TREE_COMPOSITE_ID, INVYR) |> 
  rename(YEAR = INVYR)
tree_annual
