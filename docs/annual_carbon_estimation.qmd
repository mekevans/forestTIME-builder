...
- `DECAYCD` only applies to standing dead trees
- dead trees shouldn't continue growing (although they do legitimately shrink sometimes, not sure what to do about that)

::: callout-important
## Question

It seems pretty clear that DIA shouldn't change after a tree's death, but would HT and ACTUALHT change for a standing dead tree?  Probably?  However, we don't want to extrapolate negative (or even 0?) values.
...
```
E.g. this tree went from 40 to 18 in HT between 2008 and 2013 and then was recorded as "no status" in 2020 so extrapolated values become negative.

:::

I'm pretty sure all of this has to be done *after* choosing a method to estimate mortality, so this is a branching point that will create two tables.

```{r}

#test tree
# tree_example <- tree_interpolated |>
#   filter(TREE_COMPOSITE_ID == "44_1_1_91_2_5")
...
tree_interpolated_midpoint <- 
  ...
  # Stop trees from changing size after they die
  # TODO not sure this is a good idea.  Standing dead trees do shrink over time.
  group_by(TREE_COMPOSITE_ID) |> 
  mutate(across(
    c(DIA, HT, ACTUALHT),
    \(x) if_else(STATUSCD == 2, NA, x)
  )) |>
  fill(DIA, HT, ACTUALHT) |> 
  # Fallen dead trees don't have dimensions I think
  mutate(across(
    c(DIA, HT, ACTUALHT),
    \(x) if_else(STANDING_DEAD_CD == 0 & STATUSCD == 2, NA, x)
  )) |> 
...
tree_interpolated_midpoint |> 
  ...
  select(TREE_COMPOSITE_ID, YEAR, STATUSCD, STANDING_DEAD_CD, DECAYCD, DIA, HT, ACTUALHT) |> 
  arrange(TREE_COMPOSITE_ID, YEAR)

#check for negative interpolated numbers
tree_interpolated_midpoint |> 
  filter(if_any(c(DIA, HT, ACTUALHT), \(x) x < 0))
```
