...
test_that("fallen dead trees get NAs correctly", {
  ...
  data <- prep_data(db) |> 
    ...
    dplyr::select(plot_ID, tree_ID, INVYR, DIA, HT, ACTUALHT, CULL, CR, MORTYR, STATUSCD, STANDING_DEAD_CD, DECAYCD, DESIGNCD, RECONCILECD)
  data_interpolated <- data |> 
    expand_data() |> 
    interpolate_data() |> 
    adjust_mortality(use_mortyr = FALSE)
  
  expect_true(all(is.na(data_interpolated |> filter(YEAR %in% 2017:2020) |> select(DIA, HT, ACTUALHT, CULL, CR))))
})
