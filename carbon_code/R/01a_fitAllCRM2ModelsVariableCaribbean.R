...
ref  <- data.table::fread("../Shared/Files/REF_SPECIES_FULL.csv",
                          select= c("SPCD", "GENUS", "JENKINS_SPGRPCD",
                                    "SFTWD_HRDWD",
                                    "CARIBBEAN",
                                    "WOOD_SPGR_GREENVOL_DRYWT",
                                    "BARK_SPGR_GREENVOL_DRYWT"))
names(ref) <- c("SPCD", "GENUS", "JENKINS_SPGRPCD", "SWHW", "CARIBBEAN", "WDSG", "BKSG")

tree <- merge(x= tree,
              y= ref[,c("SPCD", "SWHW", "CARIBBEAN")],
              by= "SPCD")

tree$k <- ifelse(tree$SWHW == 'S', 9, 11)

# caribbean species indicator - can occur in caribbean and from southern US/PR
tree$CARIBBEAN <- tree$CARIBBEAN == 'C' & tree$REGION %in% c("Puerto_Rico",
                                                             "Southern_States")
tree$CARIBBEAN <- as.integer(tree$CARIBBEAN)

# puerto rico indicator - keep them separate for fitting conus models
tree$PR <- as.integer(grepl("PR_", tree$AUTHOR))

tree <- tree[tree$CARIBBEAN == 1,]
...
stem$SPCD <- as.character(stem$SPCD2)
tree$SPCD <- as.character(tree$SPCD2)
...
# just use PR data for the total model
# pr <- total_by[[3]]
# pr <- pr[grepl('PR_', pr$AUTHOR), ]
# total_by[[3]] <- pr
...
writeIt <- function(name) {
  ...
  obj$SPCD <- as.numeric(obj$SPCD)
  
  # only want spcd and jenkins group
  # but only caribbean species (spcd > 999)
  # june 24 - group level model now w/ wdsg
  # obj <- obj[!is.na(obj$JENKINS_SPGRPCD), ]
  obj[is.na(obj$SPCD), ]$SPCD <- 0
  obj <- obj[is.na(obj$SPCD_DIVISION) & (obj$SPCD > 999 | !is.na(obj$JENKINS_SPGRPCD)),]
  obj[obj$SPCD == 0, ]$SPCD <- NA
  
  fn <- paste0("Coefs/Caribbean/", name, ".csv")
