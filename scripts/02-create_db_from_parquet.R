...
# x Use .csv's for ref data instead of parquet.  
# Use fs for listing files?
# Maybe use `glue` for better readability?
...
parquet_files <-
  paste0("data/parquet/", list.files(here::here("data", "parquet"), pattern = "plot")) |>
  paste(collapse = "', '")

# There is only one all_invyrs file.
all_invyrs_files <-
  paste0("data/parquet/", list.files(here::here("data", "parquet"), pattern = "all_invyrs")[1])
...
plot_query <- 
  paste0("CREATE TABLE plot AS SELECT * FROM read_parquet(['",
         parquet_files, "'])")
...
all_invyrs_query <- 
  paste0("CREATE TABLE all_invyrs AS SELECT * FROM read_parquet(['",
         all_invyrs_files, "'])") |>
  gsub(pattern = "plot", replacement = "all_invyrs")

# ref_species_file <- 
#   paste0("data/parquet/", list.files(here::here("data", "parquet"), pattern = "ref_species"))[1]

ref_species_query <- 
  paste0("CREATE TABLE ref_species AS SELECT * FROM read_csv(['data/rawdat/REF_SPECIES.csv'])")

# ref_tree_decay_prop_file <- 
#   paste0("data/parquet/",
#          list.files(here::here("data", "parquet"), pattern = "ref_tree_decay_prop"))[1]

ref_tree_decay_prop_query <- 
  paste0("CREATE TABLE ref_tree_decay_prop AS SELECT * FROM read_csv(['data/rawdat/REF_TREE_DECAY_PROP.csv'])")

# ref_tree_carbon_ratio_dead_file <- 
#   paste0("data/parquet/", 
#          list.files(here::here("data", "parquet"), pattern = "ref_tree_carbon_ratio_dead"))[1]

ref_tree_carbon_ratio_dead_query <- 
  paste0("CREATE TABLE ref_tree_carbon_ratio_dead AS SELECT * FROM read_csv(['data/rawdat/REF_TREE_CARBON_RATIO_DEAD.csv'])")
